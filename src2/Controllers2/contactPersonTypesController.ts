...
export class contactPersonTypesController {
  async index(): Promise<ContactPersonType[]> {
    try {
      ...
      const result = await pool.request().execute('[dbo].[p_GetcontactPersonTypes]');
  ...
  async getContactPersonTypeByID(id: number): Promise<ContactPersonType> {
    try {
      //@ts-ignore
      const pool = await new sql.ConnectionPool(sqlConfig).connect();
      const result = await pool.request().input('ID', sql.BigInt, id).execute('[dbo].[p_GetcontactPersonTypesByID]');
  ...
  async create(C: ContactPersonType): Promise<ContactPersonType> {
    try {
      //@ts-ignore
      const pool = await new sql.ConnectionPool(sqlConfig).connect();
      const result = await pool.request().input('contactPersonType', sql.NVarChar, C.contactPersonType).execute('[dbo].[p_SavecontactPersonTypes]');
      pool.close();
      return result.recordset[0];
    } catch (err) {
  ...
  async delete(id: number): Promise<string> {
    try {
      //@ts-ignore
      const pool = await new sql.ConnectionPool(sqlConfig).connect();
      const result = await pool.request().input('ID', sql.BigInt, id).execute('[dbo].[p_DeletecontactPersonTypes]');
      pool.close();
