...
export class AddressesController {
  async index(): Promise<Address[]> {
    try {
      ...
      const result = await pool.request().execute('[dbo].[p_GetAddresses]');
  ...
  async getAddressByID(id: number): Promise<Address> {
    try {
      //@ts-ignore
      const pool = await new sql.ConnectionPool(sqlConfig).connect();
      const result = await pool.request().input('ID', sql.Int, id).execute('[dbo].[p_GetAddressesByID]');
  ...
  async delete(id: number): Promise<string> {
    try {
      //@ts-ignore
      const pool = await new sql.ConnectionPool(sqlConfig).connect();
      const result = await await pool.request().input('ID', sql.BigInt, id).execute('[dbo].[p_DeleteAddresses]');
  ...
  async getBusinessLocations(id: number): Promise<Address[]> {
    try {
      //@ts-ignore
      const pool = await new sql.ConnectionPool(sqlConfig).connect();
      const result = await pool.request().input('subAccountID', sql.Int, id).execute('[dbo].[p_GetBusinessLoactionsBysubAccountID]');
      return result.recordset;
    } catch (err) {
      throw new Error(`Could not get all Addresses. Error: ${err}`);
    }
