...
export class ShipmentTypesController {
  ...
  async deactivate(ID: number): Promise<string> {
    try {
      const result = await De_Activate<ShipmentTypesModel>(ShipmentTypes, ID, 'deactivate');
      return result;
    } catch (err) {
  ...
  async activate(ID: number): Promise<string> {
    try {
      const result = await De_Activate<ShipmentTypesModel>(ShipmentTypes, ID, 'activate');
      return result;
    } catch (err) {
