...
export class LocationsController {
    ...
    async update(locations: LocationsModel): Promise<LocationsModel | string> {
        try {
            return await sequelize.transaction(async (t) => { // start managed transaction and pass transaction object to the callback function
                ...
                const result = await getById(Number(locations.ID));
    ...
    async deactivate(ID: number): Promise<string> {
        try {
            const result = De_Activate<LocationsModel>(Locations, ID, 'deactivate');
            return result;
        } catch (err) {
    ...
    async activate(ID: number): Promise<string> {
        try {
            const result = De_Activate<LocationsModel>(Locations, ID, 'activate');
            return result;
        } catch (err) {
