...
const getAll = async (req: Request, res: Response) => {
    try {
        ...
        const result = await departmentsController.index(language);
        res.json(result);
    } catch (error) {
        res.status(400);
        res.json(error);
...
const getAllDeActivated = async (req: Request, res: Response) => {
    try {
        const language = req.headers['accept-language'] === 'ar' ? 'ar' : 'en';
        const result = await departmentsController.indexDeActivated(language);
        res.json(result);
    } catch (error) {
        res.status(400);
        res.json(error);
...
const departmentsRouter = (app: express.Application) => {
    ...
    app.get('/departments/de-activated', getAllDeActivated);
    app.get('/departments/:ID', getById);
