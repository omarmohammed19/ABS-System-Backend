...
const getByID = async (pickupID: number, language: string, t: Transaction) => {
  const query = 'EXEC [dbo].[p_GET_ship_PickupHistory] @language = :language , @Method = :Method, @pickupID = :pickupID';
...
export class PickupHistoryController {
  async index(language: string, isActive: number, limits?: number): Promise<PickupHistoryModel[]> {
    ...
    try {
      const query = 'EXEC [dbo].[p_GET_ship_PickupHistory] @limit = :limit ,@language = :language , @Method = :Method , @isActive = :isActive';
