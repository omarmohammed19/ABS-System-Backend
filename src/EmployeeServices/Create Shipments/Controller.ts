...
const checkRefExists = async (Ref: string) => {
  const result: any = await Transactions.findOne({ where: { Ref: Ref || null } });
      ...
      newPickup = await Pickups.create(
        {
          ...
          branchID:
            data[0].Service !== 'Return'
              ? await getBranchIDByPickupLocationID(pickup.pickupLocationID)
              : await getBranchIDByReturnLocationID(pickup.returnLocationID),
    ...
    try {
      if (transactions.serviceID !== 3) {
        ...
        const result = await sequelize.transaction(async (t) => {
          const AWB = transactions.AWB ? transactions.AWB : await generateAWB(transactionHdr.subAccountID);
          ...
          if (serviceTypeIDs.length > 0) {
            for (const element of serviceTypeIDs) {
              await ShipmentServices.create(
                {
                  AWB: newTransaction.AWB,
                  serviceID: element,
        ...
        const result = await sequelize.transaction(async (t) => {
          ...
          const transactionData = [
            {
              transHdrID: newTransactionHdr.ID,
              AWB: transactions.AWB ? transactions.AWB : await generateAWB(transactionHdr.subAccountID),
              Ref: transactions.Ref,
              mainAccountID: transactions.mainAccountID,
              subAccountID: transactions.subAccountID,
              serviceID: 1,
