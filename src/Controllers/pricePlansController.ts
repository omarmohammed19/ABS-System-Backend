...
export class PricePlansController {
  async index(): Promise<PricePlan[]> {
    try {
      ...
      const result = await pool.request().execute('p_GetpricePlans');
      pool.close();
  ...
  async addPricePlan(pp: PricePlan): Promise<string> {
    try {
      ...
      pool.close();
      return 'Price Plan added successfully';
    } catch (error) {
  ...
  async deletePricePlan(id: number): Promise<string> {
    try {
      //@ts-ignore
      const pool = await new sql.ConnectionPool(sqlConfig).connect();
      const result = await pool.request().input('ID', sql.BigInt, id).execute('p_DeletepricePlans');
      pool.close();
  ...
  async getPricePlanByID(id: number): Promise<PricePlan[]> {
    try {
      //@ts-ignore
      const pool = await new sql.ConnectionPool(sqlConfig).connect();
      const result = await pool.request().input('ID', sql.BigInt, id).execute('p_GetpricePlansByID');
      pool.close();
      return result.recordset;
    } catch (error) {
      throw new Error(`Could not get pricePlans ${error}`);
